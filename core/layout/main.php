<?php
header('Cache-Control:private, no-store, must-revalidate');header('Content-Language: fr');header('Vary: Accept-Encoding');header('X-Frame-Options: SAMEORIGIN');header('X-XSS-Protection: 1; mode=block');header('X-UA-Compatible: IE=edge');header('Content-Type: text/html; charset=utf-8');ob_start('ob_gzhandler');
...
<head>
	...
	<?php
	$layout->showMetaTitle();
	$layout->showMetaDescription();
	$layout->showVendor();
	?>
...
if($this->getData(['theme', 'menu', 'position']) === 'body-first' || $this->getData(['theme', 'menu', 'position']) === 'body-top' ):
?>
